...
import { Await, defer, useAsyncValue, useLoaderData, useNavigate, useSearchParams } from "react-router-dom";
...
export function loader({ request }: { request: any }) {
  ...
  if (search) {
    return defer({
      result: fetch(`https://dummyjson.com/products/search?q=${search}`)
        .then((res) => res.json())
  ...
  if (category && category !== "all") {
    return defer({
      result: fetch(`https://dummyjson.com/products/category/${category}?limit=${defaultLimit}&skip=${skip}`)
        .then((res) => res.json())
        .then(processResponse),
    });
  }
  return defer({
    result: fetch(`https://dummyjson.com/products?limit=${defaultLimit}&skip=${skip}`)
      .then((res) => res.json())
      .then(processResponse),
  });
}
...
export function DashboardSkeleton() {
  return (
    <section className="mt-14 px-4 grid grid-cols-[repeat(auto-fill,minmax(250px,1fr))] gap-4">
      {[...Array(defaultLimit)].map(() => (
...
export function DashboardLoader({ children }: { children: React.ReactNode }) {
  const { result } = useLoaderData() as { result: ProductsResponse };
  return <Await resolve={result}>{children}</Await>;
}

export default function Dashboard() {
  const { products, total, skip } = useAsyncValue() as ProductsResponse;
